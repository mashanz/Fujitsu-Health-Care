...
#=====================================================================
def f3_knowladge_check(coordinat,fuzzy_value_a,fuzzy_value_b,output):
 ...
 if (fuzzy_value_a[coordinat[0]]>0 and fuzzy_value_b[coordinat[1]]>0):
  ...
  output.extend([[knowladge_F3[coordinat[0]][coordinat[1]],out]])
  #print(output)
...
def f3_centroid(resolution,f_out,output):
 #print(f_out,output)
 i=resolution
 s1=0
 s2=0
...
#=====================================================================
# f3 working as OVERWEIGH DETECTION
#=====================================================================
def f3_function(resolution,a,b,c,in_a,in_b,in_c,out,f3):
#=====================================================================
# INPUT AGE FUZZY
#=====================================================================
 i=0
...
#=====================================================================
 ...
 while(1):
  ...
  x8.extend([in_c[i+1]])
  x9.extend([in_c[i+2]])
  y7.extend([trapesiumL(x7[i],in_c[0],in_c[1],in_c[2])])
  y8.extend([triangle(x8[i],in_c[1],in_c[2],in_c[3])])
  y9.extend([trapesiumR(x9[i],in_c[2],in_c[3],in_c[4])])
  i+=1
  if(i==3):	
   i=0
   break
 #print(x1, y1, x2, y2, x3, y3)
 plt.figure("f3_c")
 plt.plot(x7, y7, x8, y8, x9, y9)
 plt.axvline(c, color='black')
 plt.axis([in_c[0],in_c[4], 0, 1.1])
 plt.ylabel('WEIGH')
...
#=====================================================================
 z0=[]
 z1=[]
 print("F0       :",type_0(a,in_a)) # read and calculate fuzzy value
 print("F1       :",type_0(b,in_b)) # read and calculate fuzzy value
 print("F2       :",type_0(c,in_c)) # read and calculate fuzzy value
 f3_out_ab(type_0(a,in_a),type_0(b,in_b),z0)
 #print("ALL      :",z0)
 print("MOST     :",counterMAX(z0,z1))
 #print("CENTROID :",f3_centroid(resolution,out,z1))
 f3_out = f3_centroid(resolution,out,z1)
 global f3_out
 f3=result(counterMAX(z0,z1),f3_centroid(resolution,out,z1))
 print("\n",f3)
#=====================================================================
...
#=====================================================================
 ...
 #plt.axvline(f3_centroid(resolution,out,z1), color='black')
 plt.axis([out[0],out[4], 0, 1.1])
 plt.ylabel('OVERWEIGH')
